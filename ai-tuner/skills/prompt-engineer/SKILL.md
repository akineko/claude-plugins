...
現行の Claude は指示を字義通り解釈し、勝手な一般化をしない。広く適用してほしいなら範囲を明示する。
...
現行の Claude は強い言葉に過敏に反応する。`CRITICAL: You MUST...` のような語気は過剰トリガー・過剰実行を招く。普通の指示で書く。
...
**セルフチェックは「具体的な合格基準」だけを書く**

現行の Claude（Opus 5）は指示なしで自分の仕事を検証するため、汎用の検証指示（「double-check せよ」「最終検証ステップを入れよ」「サブエージェントで検証せよ」）は過剰検証を招きトークンと遅延を増やす。既存文書からの移行では**書き換えでなく削除**が公式推奨。書く価値があるのは、モデルが自力で導出できない具体的な合格基準（フォーマット要件・降格ルール）だけ：

> 報告を出力する前に、検出した High 指摘がそれぞれ `ファイル名:行番号` と具体的な修正案を持っているか確認する。満たさないものは Medium に降格するか除去する。

（GPT 系など他社モデル向けには、出力直前の汎用セルフチェック指示も定型ミスを減らす低コストのテクニックとして有効）
...
現行の Claude は adaptive thinking（思考の要否と量をモデル自身が調整）がデフォルトで有効（Opus 5 / Sonnet 5 は `thinking` 省略時オン、Fable 5 / Mythos 5 は常時オンで無効化不可）。**thinking が動いている文脈で CoT を強制すると、不要な思考を誘発してトークンを浪費する**ため、Claude 向けの普通のプロンプトでは基本的に書かない。
...
なお、最終 assistant ターンの prefill（部分的な assistant メッセージからの続き生成）は現行の Claude ではサポート外（400 エラー）。フォーマット強制は structured outputs か直接指示へ移行する。
