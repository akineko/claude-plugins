...
description: >
  3つの設計視点（理想設計・現実設計・ドメイン設計）で並列に設計案を生成し、
  統合して最適な実装設計書を作成するオーケストレーター。
  「マルチ設計して」「multi-perspective designを作成して」「3視点で設計書を書いて」と言われたときに使用する。
...
修正内容が現在のコードベース情報（影響分析書またはResearch結果）ではカバーできない場合（新たな技術領域やサービスが関係する場合）、`code-investigation-specialist` で追加調査を実行してから 5.3 を行う。
