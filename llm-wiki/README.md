...
| **Wiki** | LLM が完全に所有するマークダウン群。サマリ・概念ページ・index・log |
| **スキーマ** | `<wiki-root>/CLAUDE.md`（または `AGENTS.md`）。Wiki の構造・ワークフロー・命名規則を定義する設定ドキュメント |
...
| `/llm-wiki:wiki-schema` | 用途追加・運用見直しのために `CLAUDE.md`（スキーマ）をカスタマイズする |
...
| `/llm-wiki:wiki-lint` | 矛盾・古い記述・孤立ページ・欠落リンクなどを検出し、改善案を提示する |
...
- **スキーマは育てる**: `CLAUDE.md` はテンプレートからスタートして、運用しながら更新していく
