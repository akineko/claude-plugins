...
`scripts/build.mjs render` に渡す分析結果の形式。**メインエージェントが書くのは「モデル」列のフィールドだけ**。統計・ユニットの全 diff・生成日時はスクリプトが map から充填する（コードの転記をモデルにさせないため。ポイントの `diffs` 抜粋だけは分析エージェントがパッチから転記する）。
...
render は必須フィールド・語彙・id/no の重複・`diffs` の件数（1〜3 件。選び方の正準は分析エージェント定義）を検証し、エラー時は生成しない。旧形式の `diff`（単数文字列）はエラーになる。さらに **map の全ファイルが units[].files か noise[].files のどちらかに割り当てられていないとエラー**になる（取りこぼし防止）。`--allow-uncovered` を付けた場合のみ、未割り当てファイルを「未分類の変更（自動追加）」グループとして明示した上で生成する。
